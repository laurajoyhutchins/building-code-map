...
Use the repo wrapper `./tools/deciduous.cmd` to build the graph. Run Deciduous commands in the current directory (not inside the source repo).
...
# Version management
./tools/deciduous.cmd check-update         # check if update is needed (version checking is always-on)
```
