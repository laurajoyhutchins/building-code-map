...
╚══════════════════════════════════════════════════════════════╝
Run './tools/deciduous.cmd check-update' for details, then 'cargo install deciduous' to upgrade.
After upgrading, run './tools/deciduous.cmd update' to refresh integration files.
        ...
        if (fs.existsSync(checkFile)) {
          ...
          if (now - lastCheck < 86400) {
            ...
            if (fs.existsSync(cachedFile)) {
              ...
              const versionResult = await $`./tools/deciduous.cmd --version`.quiet().nothrow()
