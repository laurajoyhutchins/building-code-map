...
func NormalizeQuery(query Query) (NormalizedQuery, error) {
	...
	if pointProvided == addressProvided {
		return NormalizedQuery{}, EngineError{
			Code:    ErrorInvalidQuery,
			Message: "exactly one of point or address is required",
		}
	}
	...
	if err != nil {
		return NormalizedQuery{
				...
				Code:    ErrorInvalidQuery,
				Message: "applicability_date must use YYYY-MM-DD",
				Details: map[string]any{"value": applicabilityDate},
			}
	}
