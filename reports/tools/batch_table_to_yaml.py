...
def main() -> int:
    parser = argparse.ArgumentParser(description="Batch convert a markdown table section to YAML files.")
    parser.add_argument("--heading", default="### 2.1 Primary Building Code Authorities")
    parser.add_argument("--root-key", default="authority_structure")
    parser.add_argument("--output-dir", default=str(Path(__file__).resolve().parent.parent / "yaml" / "authority-structure"))
    parser.add_argument("--yaml-template", default="{stem}_authority_structure.yaml")
    parser.add_argument(
        "--best-effort",
        action="store_true",
    ...
    return 0 if skipped == 0 or args.best_effort else 1
