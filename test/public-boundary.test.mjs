...
const policy = {
  ...
  material_classes: {
    alpha_material: { public_repository: 'review_required' },
    production_regulatory_corpus: { public_repository: 'prohibited' },
    restricted_standard_model_code: { public_repository: 'prohibited' },
  },
};

test('alpha material cannot be admitted to the public repository by default', () => {
  assert.deepEqual(validatePolicy(policy), []);
  const unsafe = structuredClone(policy);
  unsafe.material_classes.alpha_material.public_repository = 'allowed';
  assert.match(validatePolicy(unsafe).join('\n'), /alpha_material.*review_required/);
});
